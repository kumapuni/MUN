...
export default function ControlApp() {
  ...
  const currentTimer = useMemo(
    () => formatDuration(updateTimerState(state.timer).remaining),
    [state.timer]
  );
  ...
  const voteCounts = state.attendance.reduce(
    (acc, member) => {
      const vote = member.votes?.[currentVoteKey];
  ...
  const previewState = useMemo(
    () => ({
      ...state,
      currentVoteKey,
      currentVoteTabLabel: VOTE_TABS[activeVoteTab],
      fileView: { ...state.fileView, dataUrl: filePreviewUrl }
    }),
    [state, filePreviewUrl, currentVoteKey, activeVoteTab]
  );
    ...
    <div className="app-layout">
      <aside className="control-panel">
        <div className="panel-header">
          ...
          <div className="header-actions">
            ...
            <button className="secondary" onClick={exportMinutes}>
              議事録をエクスポート
            </button>
          </div>
        </div>
        ...
        <section className="panel-section">
          ...
          <div className="button-grid">
            {AVAILABLE_VIEWS.map((view) => (
              <button
                key={view.id}
                className={state.currentView === view.id ? "accent" : "secondary"}
          ...
          <div className="timer-row">
            <label>
              ...
              <input
                ...
                onChange={(event) => updateState({ timerLabel: event.target.value })}
                placeholder="例: モデレート"
              />
            </label>
          </div>
                ...
                {(state.motions ?? []).map((motion) => (
                  <tr key={motion.id}>
                    <td>
                      <input
                        className="text-input"
                        value={motion.country}
                        onChange={(event) =>
                          updateMotion(motion.id, { country: event.target.value })
                        }
                      />
                    </td>
                    <td>
                      <input
                        ...
                        value={motion.motion}
                        onChange={(event) =>
                          updateMotion(motion.id, { motion: event.target.value })
                        }
                      />
                    </td>
                    <td>
                      <input
                        ...
                        value={motion.time}
                        onChange={(event) =>
                          updateMotion(motion.id, { time: event.target.value })
                        }
                      />
                    </td>
                    <td>
                      <input
                        ...
                        onChange={(event) =>
                          updateMotion(motion.id, { direction: event.target.value })
                        }
                      />
                    </td>
                    <td>
                      <button className="ghost" onClick={() => removeMotion(motion.id)}>
                        削除
                      </button>
                    </td>
                  </tr>
      ...
      <main className="preview-panel">
        <div className="preview-header">
          ...
          <p>
            現在の画面:{" "}
            {AVAILABLE_VIEWS.find((v) => v.id === state.currentView)?.label}
          </p>
        </div>
